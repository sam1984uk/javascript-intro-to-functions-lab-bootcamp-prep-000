function shout(string){return string.toUpperCase()}

function whisper(string){return string.toLowerCase()}

function logShout(string){ var x = string.toUpperCase(); console.log(x); }

function logWhisper(string){var x = string.toLowerCase(); console.log(x);}

function sayHiToGrandma(string){var lowercase = lowercase.toLowerCase() === lowercase; if(lowercase){return `I can't herar you!`;} else if === "I love you, Grandma!"{return `I love you, too`;} else {return `YES, INDEED!`;}